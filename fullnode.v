...
Lemma about_all_validators :
  NoDup all_validators /\
  ((fold_right (fun v r => (proj1_sig (weight v) + r)%R) 0%R) all_validators > (proj1_sig t_full))%R. 
Proof.
...
Proof.
  ...
  - (* Arbitrary v0 is an equivocating sender in the new state *)
    ...
    destruct H_equiv as [msg' [H_in' H_equiv]].         (* This message has an equivocation partner *)
...
<<<<<<< HEAD
  ...
  assert (H_incl : incl (equivocating_senders s) vs) by admit.
  assert (H_piece : (sum_weights (equivocating_senders s) + sum_weights (set_diff compare_eq_dec vs (equivocating_senders s)) = sum_weights vs)%R) by admit.  
  unfold potentially_pivotal_state.
  destruct (classic (In v (equivocating_senders s))).
  (* In the case that v is not yet equivocating in s *) 
  2 : {
    exists v. 
    split. subst. assumption.
    exists (set_diff compare_eq_dec vs (equivocating_senders s)).
    repeat split.
    now apply set_diff_nodup.
    subst. rewrite set_diff_iff.
    intros [H_absurd1 _].
    contradiction.
    intros. rewrite set_diff_iff in H0.
    destruct H0.
    intro H_absurd.
    spec H_incl v0 H_absurd.
    contradiction.
    rewrite H_piece. assumption.
    rewrite H_piece. assumption. }
...
<<<<<<< HEAD
  ...
  split.
  exists (get_estimate s1,v,s1); easy. 
...
<<<<<<< HEAD
    ...
    (* We have to say that v will be an equivocating sender inside s *)
    ...
    assert (H_weights3 := fault_weight_state_incl s3 s H_in3_copy).
    (* s1' has the same fault weight as s1 *)
    (* s3 has the fault weight of s1 plus all the weights of senders in vs *)
    assert (H_s3_weight := next_equivocations_add_weights s1').
    spec H_s3_weight.
    { subst; repeat apply copy_protocol_state; try assumption; apply get_estimate_correct. }       
    spec H_s3_weight vs v H_nodup.
    rewrite H_s_inter_weight in H_s3_weight. 
    spec H_s3_weight H_under.
    spec H_s3_weight.
    intros. assert (H_disjoint_copy := H_disjoint).
    spec H_disjoint v0 H. split.
    firstorder.
    (* This is true because? *)
    admit.
    destruct H_s3_weight as [_ H_s3_weight].
    rewrite <- Heqs3 in H_s3_weight.
