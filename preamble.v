...
Lemma inhabited_compose {X Y : Type} `{StrictlyComparable X} `{StrictlyComparable Y} :
  exists (p : X * Y), True.
Proof.
  destruct (@inhabited X) as [x _]. assumption. 
  destruct (@inhabited Y) as [y _]. assumption.
  exists (x, y). 
  auto.
Qed.
