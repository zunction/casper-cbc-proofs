...
We leverage Coq's type classes to represent CBC Casper at various levels of abstrplan.
We construct a type class hierarchy showing the relationship between the various abstrplans and the properties they derive.
...
We found Coq's type classes to be a suitable abstrplan to present our findings. Our reasons are twofold: 1) the relationship between type class and instances of it aptly reflects the relationship between CBC Casper and its members; 2) Coq's type inference mechanism allows us to organize typeclasses in relation to one another and thereby elucidate the relationship between assumptions and properties. We present the our formal abstrplan using type classes in the following section.
...
\end{coq}
This completes the parameters and axioms included in our first abstrplan layer, which we call \verb|CBC_protocol_eq|. Next, we show how this abstrplan layer derives all the asynchronous, Byzantine-fault tolerant consensus safety theorems for CBC Casper.
...
With \verb|CBC_protocol_eq| as a starting point, we found that it was possible to further abstract away protocol-specific parameters while preserving the desired safety properties, albeit in more abstract terms. One particular abstrplan we explored was a partial order, formally defined as follows:
\begin{coq}
	Class PartialOrder :=
	{ A : Type;
