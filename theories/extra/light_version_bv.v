...
Variable nat256_eq : nat256 -> nat256 -> Prop.
Axiom nat256_equality_predicate : forall x y,
  nat256_eq x y <-> x = y.
Lemma nat256_eq_equiv : Equivalence nat256_eq.
Proof.
  constructor.
  - unfold Reflexive. intro. rewrite nat256_equality_predicate . reflexivity.
  - unfold Symmetric. intros. rewrite nat256_equality_predicate in * . subst. reflexivity.
  - unfold Transitive.  intros. rewrite nat256_equality_predicate in * . subst. reflexivity.
Qed.

Variable nat256_lt : nat256 -> nat256 -> Prop.
Axiom nat256_lt_strorder : StrictOrder nat256_lt.

Lemma nat256_lt_compat :  Proper (nat256_eq ==> nat256_eq ==> iff) nat256_lt.
Proof.
  constructor; rewrite nat256_equality_predicate in *; subst; intro; assumption.
Qed.

Variable hash : C * V * nat256 -> nat256.

Axiom hash_injective : forall c1 c2 v1 v2 h1 h2, 
  hash(c1,v1,h1) = hash(c2,v2,h2) -> c1 = c2 /\ v1 = v2 /\ h1 = h2. 
...
Module OWL.
Definition t := nat256.
Definition eq := nat256_eq.
Definition eq_equiv := nat256_eq_equiv.
Definition lt := nat256_lt.
Definition lt_strorder := nat256_lt_strorder.
Definition lt_compat := nat256_lt_compat.
